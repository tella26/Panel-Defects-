...
It uses the concept of Squeeze-and-Excitation (SE) block that helps to model the interdependencies between
channel of images so that it can be sensitive to features. The squeeze operation or global information embedding involves
converting feature maps into single value for each channel using global average pooling. The excitation operation or
adaptive recalibration is then used to convert the single value to a channel weight of the same size as the input. The SE blocks
are stacked with other architectures like ResNet and Inception that utilize residual and inception module respectively
